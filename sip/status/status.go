...
const (
	...
	// RFC_3261
	Queued = 182 // : "Queued",
	// Indicates that the destination was temporarily unavailable, so the server has queued the call until the destination is available.
	...
	// The original Request-URI is no longer valid, the new address is given in the Contact header field,
	...
	// The server will not accept the request without a valid Content-Length. RFC_2543
	// Deprecated by omission from later RFCs RFC_3261 and by non-registration with the IANA.
	...
	// The server was unable to validate a certificate for the domain that signed the request.<ref name="RFC_4474"/>{{rp|p11}}
	InvalidIdentityHeader = 438 // : "InvalidIdentityHeader",
	// The server obtained a valid certificate that the request claimed was used to sign the request, but was unable to verify that signature.<ref name="RFC_4474"/>{{rp|p12}}
	FirstHopLacksOutboundSupport = 439 // : "FirstHopLacksOutboundSupport",
	// The first outbound [[Session Initiation Protocol#Proxy server|proxy]] the user is attempting to register through does not
	...
	// If a SIP proxy determines a response context has insufficient Incoming Max-Breadth to carry out a desired parallel fork,
	...
	// If a SIP UA receives an INFO request associated with an Info Package that the UA has not indicated willingness to receive,
	...
	// The source of the request did not have the permission of the recipient to make such a request.<ref name="RFC_5360">{{cite IETF
	//| section      = 5.9.2
	TemporarilyUnavailable = 480 // : "TemporarilyUnavailable",
	// Callee currently unavailable.<ref name="RFC_3261"/>{{rp|§21.4.18}}
	TransactionDoesNotExist = 481 // : "TransactionDoesNotExist",
	// Server received a request that does not match any dialog or transaction.<ref name="RFC_3261"/>{{rp|§21.4.19}}
	LoopDetected = 482 // : "LoopDetected",
	// Server has detected a loop.<ref name="RFC_3261"/>{{rp|§21.4.20}}
	TooManyHops = 483 // : "TooManyHops",
	// Max-Forwards header has reached the value '0'.<ref name="RFC_3261"/>{{rp|§21.4.21}}
	AddressIncomplete = 484 // : "AddressIncomplete",
	// Request-URI incomplete.<ref name="RFC_3261"/>{{rp|§21.4.22}}
	Ambiguous = 485 // : "Ambiguous",
	// Request-URI is ambiguous.<ref name="RFC_3261"/>{{rp|§21.4.23}}
	BusyHere = 486 // : "BusyHere",
	// Callee is busy.<ref name="RFC_3261"/>{{rp|§21.4.24}}
	RequestTerminated = 487 // : "RequestTerminated",
	// Request has terminated by bye or cancel.<ref name="RFC_3261"/>{{rp|§21.4.25}}
	NotAcceptableHere = 488 // : "NotAcceptableHere",
	// Some aspect of the session description or the Request-URI is not acceptable.<ref name="RFC_3261"/>{{rp|§21.4.26}}
	BadEvent = 489 // : "BadEvent",
	// The server did not understand an event package specified in an Event header field.<ref name="RFC_3265"/>{{rp|§7.3.2}}<ref name="RFC_6665"/>{{rp|§8.3.2}}
	RequestPending = 491 // : "RequestPending",
	// Server has some pending request from the same dialog.<ref name="RFC_3261"/>{{rp|§21.4.27}}
	Undecipherable = 493 // : "Undecipherable",
	// Request contains an encrypted MIME body, which recipient can not decrypt.<ref name="RFC_3261"/>{{rp|§21.4.28}}
	...
	// The server could not fulfill the request due to some unexpected condition.<ref name="RFC_3261"/>{{rp|§21.5.1}}
	...
	// The server is acting as a [[Gateway (computer program)|gateway]] or [[Proxy server|proxy]], and received an invalid response from a downstream server while attempting to fulfill the request.<ref name="RFC_3261"/>{{rp|§21.5.3}}
	ServiceUnavailable = 503 // : "ServiceUnavailable",
	// The server is undergoing maintenance or is temporarily overloaded and so cannot process the request. A "Retry-After" header field may specify when the client may reattempt its request.<ref name="RFC_3261"/>{{rp|§21.5.4}}
	ServerTimeout = 504 // : "ServerTimeout",
	// The server attempted to access another server in attempting to process the request, and did not receive a prompt response.<ref name="RFC_3261"/>{{rp|§21.5.5}}
	VersionNotSupported = 505 // : "VersionNotSupported",
	// The SIP protocol version in the request is not supported by the server.<ref name="RFC_3261"/>{{rp|§21.5.6}}
	MessageTooLarge = 513 // : "MessageTooLarge",
	// The request message length is longer than the server can process.<ref name="RFC_3261"/>{{rp|§21.5.7}}
	PushNotificationServiceNotSupported = 555 // : "PushNotificationServiceNotSupported",
	// The server does not support the push notification service identified in a 'pn-provider' SIP URI parameter<ref name="RFC_8599">{{cite IETF
	...
	// All possible destinations are busy. Unlike the 486 response, this response indicates the destination knows there are no alternative destinations (such as a voicemail server) able to accept the call.<ref name="RFC_3261"/>{{rp|§21.6.1}}
	Decline = 603 // : "Decline",
	...
	// The server has authoritative information that the requested user does not exist anywhere.<ref name="RFC_3261"/>{{rp|§21.6.3}}
	...
	// The called party did not want this call from the calling party. Future attempts from the calling party are likely to be similarly rejected.<ref name="RFC_8197">{{cite IETF
